...
def preprocess_data(
    ...
    #     datasets.
    TRANSFORMACIONES_CAT = [('ordinal_enc', OrdinalEncoder(), 
    ...
    transformer = ColumnTransformer(transformers = TRANSFORMACIONES_CAT, remainder='passthrough')
    ...
    PIPELINE= Pipeline([('encode', transformer),
                        ('imputer', imputer),
                        ('scaler', scaler)
                        ])

    train_df= PIPELINE.fit_transform(working_train_df)
    val_df= PIPELINE.transform(working_val_df)
    test_df= PIPELINE.transform(working_test_df)
