...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_ammount_per_date = filtered_dates.groupby(['date']).agg({
        'order_id': 'count'
        }).reset_index()
    ...
    result_df = order_purchase_ammount_per_date.merge(holidays,
                                                      on = 'date',
                                                      how = 'left')
    result_df['holiday'] = np.where(result_df['name'].isnull(), False, True)
    result_df.rename(columns={"order_id": "order_count"}, inplace = True)
