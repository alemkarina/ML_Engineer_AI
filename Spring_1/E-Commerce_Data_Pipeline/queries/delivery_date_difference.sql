...
SELECT x.customer_state AS State,
CAST(avg(JULIANDAY(STRFTIME('%Y-%m-%d', y.order_estimated_delivery_date))- 
	JULIANDAY(STRFTIME('%Y-%m-%d', y.order_delivered_customer_date))) AS INT) AS Delivery_Difference
    FROM olist_customers x
    INNER JOIN olist_orders y
    ON x.customer_id = y.customer_id
    WHERE y.order_delivered_customer_date IS NOT NULL
    AND y.order_status == 'delivered'
