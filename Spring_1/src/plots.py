...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    sns.scatterplot(x='product_weight_g', y='freight_value', data=df, ax=ax)
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    a= df[df['holiday']==True]
    a[['date']]
    xcoords = a['date'].tolist()
    for xc in xcoords:
        plt.axvline(x=xc, ls='--')
        plt.legend(['order amount per day with holidays'])
